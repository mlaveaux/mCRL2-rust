...
pub fn branching_bisim_sigref(preprocessed_lts: &LabelledTransitionSystem, incoming: &IncomingTransitions, timing: &mut Timing) -> BlockPartition {
            ...
            })
        {
            if block_index != new_block_index {
                ...
                for &state_index in &states {
                    for trans in incoming.incoming_transitions(state_index) {
                        if BRANCHING {
                                ...
                                || partition.block_number(trans.state()) < num_blocks
                            {
                                ...
                                partition.mark_element(trans.state());
                            }
                        } else {
