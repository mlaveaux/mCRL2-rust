...
impl LabelledTransitionSystem for LTS {
    fn initial_state_index(&self) -> StateIndex {
        self.initial_state
    }

    fn outgoing_transitions(&self, state_index: usize) -> impl Iterator<Item = &(LabelIndex, StateIndex)> {
        let state = &self.states[state_index];
        self.transitions[state.outgoing_start..state.outgoing_end].iter()
    }

    fn iter_states(&self) -> impl Iterator<Item = StateIndex> {
        0..self.states.len()
    }

    fn num_of_states(&self) -> StateIndex {
        self.states.len()
    }

    fn num_of_labels(&self) -> LabelIndex {
        self.labels.len()
    }
