...
pub fn load_case(name: &str, max_number_expressions: usize) -> (JittyRewriter, Vec<ATerm>)
{
    ...
    let expressions: Vec<ATerm> = BufReader::new(file).lines().take(max_number_expressions).map(|x| data_spec.parse(&x.unwrap())).collect();
