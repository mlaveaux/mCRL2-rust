...
fn rec_test(rec_files: Vec<&str>, expected_result: &str) 
{
    ...
    for term in &terms {
        let expected_result = expected.next().unwrap().trim_end(); // Removes the '/r' garbage that might remain on Windows platforms.

        let result = inner.rewrite(term.clone());
        assert_eq!(format!("Result: {}", &result), expected_result, "The inner rewrite result doesn't match the expected result");
