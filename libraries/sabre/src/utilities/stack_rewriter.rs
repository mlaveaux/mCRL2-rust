...
use crate::{RewritingStatistics, SabreRewriter};
use crate::set_automaton::{State, EnhancedMatchAnnouncement, EquivalenceClass};
use crate::sabre::configuration_stack::{ConfigurationStack, SideInfo, SideInfoType, Configuration};
...
impl SabreRewriter{
    ...
    fn stack_based_normalise_aux(tp: &mut TermPool, states: &Vec<State>, t: StoredTerm, stats: &mut RewritingStatistics, _gc: bool) -> StoredTerm {
        ...
        'outer: loop {
            ...
            'skip_point: loop {
                // We do not use garbage collection at the moment
                // if gc && rewrites_since_gc >= (cl.configuration_stack.len() * 200) && rewrites_since_gc > 100 {
                //     rewrites_since_gc = 0;
                //     cl.integrate_updated_subterms(0, tp, true);
                //     tp.mark_term(&cl.configuration_stack[0].subterm);
                //     tp.collect_garbage();
                // }
                ...
                if let Some(leaf_index) = cl.get_unexplored_leaf() {
                    ...
                    match ConfigurationStack::pop_side_branch_leaf(&mut cl.side_branch_stack, leaf_index) {
                        None => {//We are exploring a bud
                            //If the subterm in the configuration has a known normal form we use that
